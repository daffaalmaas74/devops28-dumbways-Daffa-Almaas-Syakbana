...
semua server sudah disetup agar tidak bisa login menggunakan password dan hanya bisa login menggunakan SSH - KEY
...
8. selanjutnya, buat user baru dengan perintah 'CREATE USER 'nama_user'@'%' IDENTIFIED BY 'isi_password';' dan lakukan reload hak akses dengan menggunakan perintah 'FLUSH PRIVILEGES;'

![Gambar 12](gambar12.png)

9. selanjutnya berikan hak akses penuh pada database wayshub terhadap user dengan perintah 'GRANT ALL PRIVILEGES ON wayshub.* TO 'nama_user'@'%';' dan lakukan reload hak akses

![Gambar 13](gambar13.png)

10. setelah berhasil, keluar dari mysql dan edit isi file konfigurasi mysqld.cnf di direktori /etc/mysql/mysql.conf.d menggunakan perintah '/etc/mysql/mysql.conf.d/mysqld.cnf', lalu ubah isi 'bind-address' dan 'mysql-bind-address'  menjadi 0.0.0.0 seperti pada gambar dibawah
...
2. lakukan clone terhadap https://github.com/dumbwaysdev/wayshub-backend.git

![Gambar 16](gambar16.png)

3. buka direktori wayshub-backend, lalu install depedensi/library yang dibutuhkan oleh project wayshub-backend dengan menggunakan perintah 'npm install'

![Gambar 17](gambar17.png)

4. Selanjutnya, buka file config/config.json menggunakan perintah nano config/config.json. Pada bagian environment development, sesuaikan username dan password dengan user yang memiliki akses ke database wayshub, ubah isi database menjadi 'wayshub', serta isi host dengan IP address server database.

![Gambar 18](gambar18.png)

5. selanjutnya, install sequelize-cli dengan perintah 'npm i -g sequelize-cli'

![Gambar 19](gambar19.png)

6. Selanjutnya, lakukan migrasi tabel ke database dengan perintah 'sequelize db:migrate'

![Gambar 20](gambar20.png)

7. Selanjutnya, install PM2 dengan perintah 'npm install -g pm2'

![Gambar 21](gambar21.png)

8. Selanjutnya, buat file konfigurasi ecosystem PM2 dengan perintah 'PM2 init' yang berfungsi menyimpan pengaturan aplikasi saat dijalankan oleh PM2.

9. edit isi file ecosystem.config.js dengan perintah 'nano ecosystem.config.js', lalu tambahkan key / property name diatas 'script' seperti gambar dibawah

![Gambar 22](gambar22.png)

10. Jalankan aplikasi diatas PM2 dengan perintah 'PM2 start ecosystem.config.js'

![Gambar 23](gambar23.png)

11. Aplikasi berhasil dijalankan di http://23.100.98.52:5000/
...
5. Selanjutnya, buka file src/config/api.js menggunakan perintah nano src/config/api.js. Pada bagian baseURL, masukkan ip adress server backend dengan tambahan port 5000 seperti gambar dibawah.

![Gambar 31](gambar31.png)

6. Selanjutnya, install PM2 dengan perintah 'npm install -g pm2'

![Gambar 32](gambar32.png)

7. Selanjutnya, buat file konfigurasi ecosystem PM2 dengan perintah 'PM2 init' yang berfungsi menyimpan pengaturan aplikasi saat dijalankan oleh PM2.
...
3. selanjutnya buka domain studentdumbways.my.id
